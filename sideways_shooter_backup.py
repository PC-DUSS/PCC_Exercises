...
class SidewaysShooter():
    ...
    def _check_projectile_collisions(self):
        ...
        if collisions:
            for each_bullet in collisions:
                for each_enemy in collisions[each_bullet]:
    ...
    def _check_highscore(self):
        if self.stats.score > self.stats.highscore:
            ...
            self.scoreboard.blit_highscore()
    ...
    def _reset_game(self):
        ...
        self.scoreboard.blit_varying_elements()
...
class Scoreboard():
    ...
    def blit_lives(self):
        ...
        for life in range(0, self.stats.player_lives):
            tmp_avatar = Avatar(self.game_instance)
            tmp_avatar.rect.x = life * tmp_avatar.rect.width + 5
            tmp_avatar.rect.y = 5
            self.avatars.add(tmp_avatar)
        self.avatars.draw(self.screen)


    def blit_varying_elements(self):
        self.blit_score()
        self.blit_level()
        self.blit_lives()

    def blit_highscore(self):
        self.highscore_img = self.font.render(str(self.stats.highscore), True,
                                              self.sb_text_color,
                                              self.settings.bg_color
                                              )
